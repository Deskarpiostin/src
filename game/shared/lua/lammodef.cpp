...
static bool lua_to_int_or_named(lua_State* L, int idx, int defaultVal, bool hasDefault, int &out)
{
    if (lua_isnoneornil(L, idx)) {
        if (hasDefault) { out = defaultVal; return true; }
        return false;
    }

    if (lua_isnumber(L, idx)) {
        out = (int)lua_tointeger(L, idx);
        return true;
    }
...
static bool lua_to_float_or_named(lua_State* L, int idx, double defaultVal, bool hasDefault, double &out)
{
    if (lua_isnoneornil(L, idx)) {
        if (hasDefault) { out = defaultVal; return true; }
        return false;
...
{
    lua_pushfstring(L, "CAmmoDef: %p", lua_touserdata(L, 1));
    return 1;
}
